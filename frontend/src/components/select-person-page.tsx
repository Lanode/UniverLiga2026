...
export function SelectPersonPage() {
  ...
  const [people, setPeople] = useState<Person[]>([]);   // ← state

  const handleCancel = () => navigate(`/task/${id}`);
  const handleClose = () => navigate(`/task/${id}`);
  ...
  useEffect(() => {
    async function getData() {
      const response = await fetch(`${server}/task/${id}/users`, {
        headers: {
          "Content-Type": "application/json",
          Authorization:
            `${localStorage.getItem("token_type")} ${localStorage.getItem(
              "access_token"
            )}`,
        },
      });
      ...
      setPeople(data);               // ← update state → re‑render
    }

    getData();
  }, [id]); // run again only if the task id changes
                  ...
                  <div className="feedback-person-row">
                    <div className="feedback-person-info">
                      ...
                      <div className="feedback-person-details">
                        <span className="feedback-person-name">
                          {person.full_name}
                        </span>
                        <span className="feedback-person-role">
                          {person.role}
                        </span>
                      </div>
                    </div>
...
/* PersonAvatar component unchanged */
  ...
  return (
    <div className="person-avatar">
      ...
      >
        ...
        <path
          d="M15.5 17.5 Q18 19.5 20.5 17.5"
          stroke="white"
          strokeWidth="1"
          fill="none"
          strokeLinecap="round"
